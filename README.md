...
> Текущая версия README рассчитана на `panel-script-v1 1.2.0`.
...
/root/panel-script-v1.sh --cascade       # включить/донастроить каскад позже
...
При установке любого CDN скрипт спрашивает:

```text
Нужен каскад через второй сервер? [y/N]
```

Если выбрать `нет`, ничего не теряется: каскад можно включить позже без переустановки панели и базового CDN:

```bash
/root/panel-script-v1.sh --cascade
```

Для Remnawave схема такая:

```text
клиент -> CDN -> RU relay -> foreign exit -> интернет
```

Мастер выбирает две уже подключённые ноды и старается автоматически подготовить панельную часть: `BRIDGE_IN :8888` на exit, relay Config Profile с CDN inbound `:7443`, `VLESS_EXIT`, routing, Internal Squad, bridge-user и отдельный cascade Host. Существующие Active Inbounds на exit сохраняются; замену активного Profile у relay скрипт делает только после отдельного подтверждения.

То, что находится вне центральной панели, выводится отдельным блоком `[ВРУЧНУЮ]`: Caddy на relay, firewall/SG для `8888`, DNS/origin у CDN-провайдера и финальные проверки. Сначала базовый CDN рекомендуется довести до `origin=400` и `CDN=400`, и только затем переключать origin на relay.
