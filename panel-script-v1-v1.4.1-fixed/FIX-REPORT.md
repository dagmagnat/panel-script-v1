# Отчёт по исправлению проекта — v1.4.1
...
- исправления предыдущих версий встроены в полный `install.sh` v1.4.1;
...
- добавлен безопасный SFTPGo frontend без второго Caddy: backend только на loopback, атомарный nginx patch и сохранение XHTTP-пути.
- добавлен безопасный `--cascade-relay-nginx`: проверка `:7443`, точечная замена старого upstream (`:4443` для Yandex), post-check и откат.
...
bash tests/frontend-tests.sh                6 checks PASS
```
